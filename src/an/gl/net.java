...
final public class net{
	final public static class cfg{
		public static String net_server_port="8889";
	...
	public static int nplayersconnected=0;
	...
	public static void main(String[] args) throws Throwable{
		...
		if(args.length>0)cfg.net_server_port=args[0];
		status="opening";
		final ServerSocketChannel serverSocketChannel=ServerSocketChannel.open();
		serverSocketChannel.configureBlocking(false);
		final InetSocketAddress inetSocketAddress=new InetSocketAddress(Integer.parseInt(cfg.net_server_port));
		...
		status="waiting for "+nplayers+" players";
		out.println(status);
		while(on){try{
			selector.select();
			...
			while(iterator.hasNext()){
				...
				if(selectionKey.isAcceptable()){
					...
					nplayersconnected++;
					status="player "+p.socketChannel.socket().getInetAddress().getHostAddress()+" connected ("+nplayersconnected+" of "+nps.length+")";
					System.out.println(status);
					if(nplayersconnected==nps.length){
						status+=" all players connected. starting";
						out.println(status);
					...
					if(n==-1)
						return;
					bbs[p.plrnbr].flip();
					status="frame "+frame+" player "+(p.plrnbr+1)+" says "+new String(bbs[p.plrnbr].array(),"utf8");
...
//							System.out.print(frameno+"\r");
	...
	public static void print_stats(final OutputStream out) throws Throwable{
		...
		ps.println("             time: "+new Date(acti.time_millis()));
		ps.println("             port: "+cfg.net_server_port);
		ps.println("          players: "+nplayersconnected+"/"+nplayers);
		final Runtime rt=Runtime.getRuntime();
		if(cfg.gc_before_stats)
			rt.gc();
